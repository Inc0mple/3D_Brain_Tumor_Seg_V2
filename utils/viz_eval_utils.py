...
def get_jaccard_scores(filename):
    ...
    for line in lines:
        if "jaccard_scores" in line:
            ...
            train_jaccard_str = train_jaccard_str.replace(
                '[', '').replace(']', '')
            val_jaccard_str = val_jaccard_str.replace('[', '').replace(']', '')

            # convert the strings to lists of floating-point numbers
            train_jaccard = [float(x.strip())
                             for x in train_jaccard_str.split(', ')]
            val_jaccard = [float(x.strip())
                           for x in val_jaccard_str.split(', ')]
            break
...
def plot_param_count(results_dict, paletteCols):
    ...
    fig.savefig("results/param_count_all.png", format="png",
...
def plot_train_losses(results_dict, paletteCols):
    ...
    sns.lineplot(data=train_loss_df[models_ls],
                 linewidth=3, palette=paletteCols)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Train Losses")
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_train_losses_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_val_losses(results_dict, paletteCols):
    ...
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_val_losses_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_train_dices(results_dict, paletteCols):
    ...
    sns.lineplot(data=train_dices_df[models_ls],
                 linewidth=3, palette=paletteCols)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Train Dice Scores")
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_train_dice_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_val_dices(results_dict, paletteCols):
    ...
    sns.lineplot(data=val_dices_df[models_ls],
                 linewidth=3, palette=paletteCols)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Validation Dice Scores")
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_val_dice_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_train_jaccards(results_dict, paletteCols):
    ...
    epoch_num = len(
        results_dict["3DOnet_DoubleConv_Kernel1"]["train_jaccards"])
    epoch_num_xlist = list(range(1, epoch_num + 1))
    ...
    sns.lineplot(data=train_jaccards_df[models_ls],
                 linewidth=3, palette=paletteCols)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Train Jaccard Scores")
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_train_jaccard_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_val_jacards(results_dict, paletteCols):
    ...
    sns.lineplot(data=val_jacards_df[models_ls],
                 linewidth=3, palette=paletteCols)
    ax.set_xlabel("Epochs")
    ax.set_ylabel("Validation Jaccard Scores")
    ax.legend()
    plt.xticks(rotation=45, ha='right')
    fig.savefig("results/trainer_val_jaccard_all.png", format="png",
                pad_inches=0.2, transparent=False, bbox_inches='tight')
    plt.show()
...
def plot_trainer_runtime(results_dict, paletteCols):
    ...
    # sort based on ascending order of trainer runtime
    x = [val for _, val in sorted(zip(y, x))]
    y = sorted(y)
...
def plot_inference_time(results_dict, paletteCols):
    ...
    # sort based on ascending order of inference time
    x = [val for _, val in sorted(zip(y, x))]
    y = sorted(y)
