...
class ONet3d_v3_DoubleConv(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        self.enc1_bot = Down(n_channels, 2 * n_channels,
                             kernel_size=3, padding=1)
        self.enc2_bot = Down(2 * n_channels, 4 * n_channels,
                             kernel_size=3, padding=1)
        self.enc3_bot = Down(4 * n_channels, 8 * n_channels,
                             kernel_size=3, padding=1)
        self.enc4_bot = Down(8 * n_channels, 8 * n_channels,
                             kernel_size=3, padding=1)

        self.enc1_top = Down(n_channels, 2 * n_channels,
                             kernel_size=1, padding=0)
        self.enc2_top = Down(2 * n_channels, 4 * n_channels,
                             kernel_size=1, padding=0)
        self.enc3_top = Down(4 * n_channels, 8 * n_channels,
                             kernel_size=1, padding=0)
        self.enc4_top = Down(8 * n_channels, 8 * n_channels,
                             kernel_size=1, padding=0)

        self.dec1_bot = Up(16 * n_channels, 4 * n_channels,
                           kernel_size=3, padding=1)
        self.dec2_bot = Up(8 * n_channels, 2 * n_channels,
                           kernel_size=3, padding=1)
        self.dec3_bot = Up(4 * n_channels, n_channels,
                           kernel_size=3, padding=1)
        self.dec4_bot = Up(2 * n_channels, n_channels,
                           kernel_size=3, padding=1)

        self.dec1_top = Up(16 * n_channels, 4 * n_channels,
                           kernel_size=1, padding=0)
        self.dec2_top = Up(8 * n_channels, 2 * n_channels,
                           kernel_size=1, padding=0)
        self.dec3_top = Up(4 * n_channels, n_channels,
                           kernel_size=1, padding=0)
        self.dec4_top = Up(2 * n_channels, n_channels,
                           kernel_size=1, padding=0)
